...
# Create session states for both selections
if 'selected_company' not in st.session_state:
    st.session_state.selected_company = None
if 'selected_treeplot_company' not in st.session_state:
    st.session_state.selected_treeplot_company = None

selected_company = st.sidebar.selectbox("Select a Company", options=treemap_df['Company'].unique())

################
# Update session state when a company is selected from the dropdown
if selected_company:
    st.session_state.selected_company = selected_company

# Treeplot click handler (Replace with actual treeplot code)
def on_treeplot_click(company_name):
    st.session_state.selected_treeplot_company = company_name
company_name = on_treeplot_click()
on_treeplot_click(company_name)
################

def create_sankey(selected_company):

    # Callback to update the graphic based on treemap click
    if selected_company:
        # Retrieve ticker based on selected company
        ticker = treemap_df[treemap_df['Company'] == selected_company]['Ticker'].iloc[0]
        ...
        if financial_metrics:
            ...
            fig.update_layout(
                hovermode='x',
                title=f"<span style='font-size:36px;color:steelblue;'><b>{selected_company} FY23 Income Statement</b></span>",
            ...
            fig.update_layout(title_text=f"{selected_company} FY23 Income Statement")
            if st.session_state.selected_company:
                sankey_fig = create_sankey(st.session_state.selected_company)
                st.plotly_chart(sankey_fig, use_container_width=True)
            with col2:
                if st.session_state.selected_treeplot_company:
                    sankey_fig_treeplot = create_sankey(st.session_state.selected_treeplot_company)
                    st.plotly_chart(sankey_fig_treeplot, use_container_width=True)